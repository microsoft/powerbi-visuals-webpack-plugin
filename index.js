...
class PowerBICustomVisualsWebpackPlugin {
  ...
  constructor(options) {
    ...
    const defaultOptions = {
      ...
      iconImage: !options.assets.icon ?
        base64Img.base64Sync(path.join(__dirname, "templates", "icon.png")) : base64Img.base64Sync(path.join(process.cwd(), options.assets.icon)),
      devMode: true,
  ...
  apply(compiler) {
    ...
    compiler.hooks.beforeRun.tapAsync(this._name, (compilationParams, callback) => {
      this._beforeCompile(callback);
    });

    compiler.hooks.watchRun.tapAsync(this._name, (compilation, callback) => {
      this._beforeCompile(callback);
    });
  }
  ...
  async _emit(compilation) {
    ...
    const config = await Promise.all([
      ...
      ([stringResources, dependencies, _, jsContent, cssContent]) => 
        this.getVisualConfig(stringResources, capabilities, dependencies, jsContent, cssContent)
    ).catch(err => {
      throw err;
    });
    ...
    compilation.assets["pbiviz.json"] =  new RawSource(JSON.stringify(config));
  ...
  async _beforeCompile(callback) {
    ...
    await this.generateVisualPlugin()
      ...
      .catch(ex => {
        ...
        ex.forEach(ex => this._error(ex.message));
  ...
  async generateVisualPlugin() {
    ...
    const pluginTemplate = await fs.readFile(path.join(__dirname, 'templates', 'plugin.ts.template'));
    const pluginTs = _.template(pluginTemplate)(pluginOptions);
    return await fs.writeFile(path.join(process.cwd(), '.tmp', 'precompile', 'visualPlugin.ts'), pluginTs);
  }
  ...
  async generateRosources(config) {
    ...
    operations.push(
      fs.outputFile(path.join(dropPath, 'package.json'), packageJSONContent, {
        encoding: ENCODING
      })
    );
    ...
    if (this.options.generateResources) {
      operations.push(
        fs.outputFile(path.join(resourcePath, 'visual.js'), config.content.js, {
          encoding: ENCODING
        }),
        fs.outputFile(path.join(resourcePath, `${prodConfig.visual.guid}.pbiviz.json`), JSON.stringify(prodConfig), {
          encoding: ENCODING
        }),
        fs.outputFile(path.join(resourcePath, 'visual.prod.js'),  config.content.js, {
          encoding: ENCODING
        }),
        fs.outputFile(path.join(resourcePath, 'visual.prod.css'), config.content.css, {
          encoding: ENCODING
        })
      );
    }
    ...
    if (this.options.generatePbiviz) {
      operations.push(this.generatePbiviz(prodConfig, packageJSONContent, this.options.packageOutPath));
    }
  ...
  async generatePbiviz(visualConfigProd, packageJSONContent, dropPath) {
    ...
    zip.folder("resources").file(`${visualConfigProd.visual.guid}.pbiviz.json`, JSON.stringify(visualConfigProd));
    const inputStream = zip.generateNodeStream();
    const outFilePath = path.join(dropPath, `${visualConfigProd.visual.guid}.${visualConfigProd.visual.version}.pbiviz`);
    ...
    return new Promise((resolve, reject) =>
      ...
      .pipe(outStream)
      .on('error', (err) => {
        utils.cleanupFileStream(inputStream, outStream, outFilePath, err);
        logger.error('Connot write package');
        reject(err);
      })
      .on('close', resolve)
    );
