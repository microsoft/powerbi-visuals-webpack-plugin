...
1. Install all required libraries for build a visual
```
...
```
npm i @babel/polyfill @babel/runtime @babel/runtime-corejs2 --save
```
...
```
installs the latest version of API package
...
Also, you can use certificates from `powerbi-visuals-toos` or use autogenerated certificates by `webpack-dev-server`. Just skip section 4 to use webpack-dev-server's certs.
...
or
```
npm i powerbi-visuals-tools@3.0.12 --save-dev
```
...
6. Add script to build a visual package

Add into `scripts` section of `package.json` command `"package": "webpack"`:
...
Run command `npm run package` to create visual package.

# Run webpack-dev-server to debug custom visual
...
Add into `scripts` section of `package.json` command `"start": "webpack-dev-server"`:
