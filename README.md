...
5. Use sample of config  webpack 5. (copy into `webpack.config.js`)
    ...
    // webpack sets process.env.WEBPACK_SERVE when running `webpack serve` (dev server).
    // Use it to switch automatically between developer-visual (dev server) and packaging behavior.
    ...
    module.exports = {
        ...
        output: {
            clean: isProduction, // do NOT clean while serving: pbiviz.json is written outside webpack's asset pipeline and would be deleted by clean
            path: path.join(__dirname, ".tmp", "drop"),
            publicPath: 'assets',
            filename: "[name]",
            library: `${pbivizFile.visual.guid}${isProduction ? "" : "_DEBUG"}`, // dev server registers the visual under `<guid>_DEBUG`
            libraryTarget: "var",
        },
        devServer: {
            static: {
            ...
            devMiddleware: {
                writeToDisk: true // required: flush emitted assets (status, visual.js, visual.css, source maps) to `.tmp/drop` so they are served under `/assets/*` (e.g. `/assets/status`)
            },
        ...
        plugins: [
            ...
            new PowerBICustomVisualsWebpackPlugin({ // custom visuals plugin instance with options
                ...
                devMode: !isProduction, // adds the `_DEBUG` suffix to the status file, plugin name and guid required by the Power BI developer visual
                generatePbiviz: true,
...
### Serving the `status` file and visual assets in development
...
1. Serves the drop folder under the `/assets/` route (`devServer.static.directory` = `.tmp/drop`, `publicPath` = `/assets/`).
2. Writes emitted assets to disk with `devMiddleware: { writeToDisk: true }`.
3. Uses `output.publicPath: 'assets'` so emitted assets resolve under `/assets/*`.
4. Keeps `output.clean` **disabled** while serving. `pbiviz.json` is written by the plugin directly to disk (it is intentionally not a webpack asset), so `clean: true` would delete it after every rebuild and `/assets/pbiviz.json` would return `404`.
5. Runs the plugin with `devMode: true` while serving, so the `status` file, plugin name and visual GUID get the `_DEBUG` suffix that the Power BI developer visual expects (e.g. `status` contains `<guid>_DEBUG` instead of `<guid>`). Keep the webpack `output.library` in sync (`<guid>_DEBUG`). When `devMode` is `true`, minification is also typically disabled, so no `visual.js.LICENSE.txt` sidecar is produced.
